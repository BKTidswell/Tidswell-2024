...
perp_angle_lmer_close <- lmer(Fish1_Yaw_Heading_PS ~ Ablation*Darkness + (1|Date), data = perp_angle_df %>% filter(Dist_Cat == "Close"))
...
perp_angle_lmer_far <- lmer(Fish1_Yaw_Heading_PS ~ Ablation*Darkness + (1|Date), data = perp_angle_df %>% filter(Dist_Cat == "Far"))
...
turning_school_stats_table <- tibble(
       Near_or_Far = rep(c(rep("Near",3) , rep("Far",3)), 1),
       Condition = rep(c("Ablation","Darkness","Ablation x Darkness"), 2),
       DF = c(rep(length(perp_angle_lmer_close@resp$y)-3,3),
                  rep(length(perp_angle_lmer_far@resp$y)-3,3)),
       Chisq = round(c(perp_angle_aov_me_close$Chisq[1:3],perp_angle_aov_me_far$Chisq[1:3]),3),
       P_Val = round(c(perp_angle_aov_me_close$`Pr(>Chisq)`[1:3],perp_angle_aov_me_far$`Pr(>Chisq)`[1:3]),3),
       Is_Sig = P_Val < 0.05)
