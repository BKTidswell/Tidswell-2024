...
glmm_school_stats_table <- tibble(Stat = c(rep("NND",4),rep("Height",4),rep("Polarization",4),rep("Speed",4),rep("Area",4),rep("Groups",4)),
       ...
       Chisq = round(c(nnd_s_aov_me$Chisq[1:2],nnd_f_aov_me$Chisq[1:2],
                 ...
                 groups_s_aov_me$Chisq[1:2],groups_f_aov_me$Chisq[1:2]),2),
       
       P_Val = round(c(nnd_s_aov_me$`Pr(>Chisq)`[1:2],nnd_f_aov_me$`Pr(>Chisq)`[1:2],
                 ...
                 groups_s_aov_me$`Pr(>Chisq)`[1:2],groups_f_aov_me$`Pr(>Chisq)`[1:2]),2),
...
turning_results <- tibble(Flow_or_Still = c("Still","Still","Still","Still","Still","Flow","Flow","Flow","Flow","Flow"),
       ...
       LR_Chisq = round(c(tgs_aov$`LR Chisq`,tgf_aov$`LR Chisq`),2),
       P_Val = round(c(tgs_aov$`Pr(>Chisq)`,tgf_aov$`Pr(>Chisq)`),2),
       Is_Sig = P_Val < 0.05)
...
single_vs_eight_results <- tibble(Condition = c("Light","Light","Ablated","Ablated","Darkness","Darkness"),
       ...
       LR_Chisq = round(c(ltg_aov$`LR Chisq`[2:3],altg_aov$`LR Chisq`[2:3],dtg_aov$`LR Chisq`[2:3]),3),
       P_Val = round(c(ltg_aov$`Pr(>Chisq)`[2:3],altg_aov$`Pr(>Chisq)`[2:3],dtg_aov$`Pr(>Chisq)`[2:3]),3),
       Is_Sig = P_Val < 0.05)
