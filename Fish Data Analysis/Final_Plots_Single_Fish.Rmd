...
```{r}
school_data <- read.csv("Data/Fish_School_Values_Single_Fish.csv")
...
ggplot(school_data, aes(x = interaction(Ablation,Darkness,sep=", "), y = NND, fill = interaction(Ablation,Darkness,sep=", ")))+
  ...
  ggtitle("Single Fish: Effect of Available Senses on Nearest Neighbor Distance (NND)") +
  xlab("") +
  ylab("NND (BL)") +
  theme_light() +
...
ggsave("Final Graphs/School_NND_Single_Fish.png")
```

### Num fish Within 2 BL

```{r}
raw_school_data <- read.csv("Data/Fish_Raw_Comp_Values_Single_Fish.csv")
...
ggplot(count_within_2, 
  ...
  guides(fill = guide_legend(title = "Condition")) +
  scale_fill_manual(values=c(light_no_ab_color, light_ab_color, dark_no_ab_color)) +
  ggtitle("Single Fish: Mean size of group within 2BL") +
  xlab("") +
...
ggsave("Final Graphs/Mean_Group_Size_Single_Fish.png")
```
...
ggsave("Final Graphs/Num_Aligned_Fish_2BL_Single_Fish.png")
...
indiv_data <- read.csv("Data/Fish_Individual_Values_Single_Fish.csv")
